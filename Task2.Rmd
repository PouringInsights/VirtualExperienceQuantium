...
setwd("C:/Giang/Studying/Virtual Internship- Quantium/Task 2")
...
#### Defining inputTable as a metric table with potential comparison stores, metricCol as the store metric used to calculate correlation on, and storeComparison as the store number of the trial store.
...
numeric())
 ...
 "Store2" = i,
"corr_measure" = cor(inputTable[STORE_NBR == storeComparison, eval(metricCol)], inputTable[STORE_NBR == i,
eval(metricCol)])
)
...
percentageDiff <- merge(scaledControlSales[, c("YEARMONTH","controlSales")], measureOverTime[STORE_NBR == trial_store,c("YEARMONTH","totSales")],
  by = "YEARMONTH", allow.cartesian = TRUE
...
scaledControlCustomers <- measureOverTimeCusts[STORE_NBR == control_store,
 ...
 ][, Store_type := ifelse(STORE_NBR == control_store, "control", ifelse(STORE_NBR== trial_store, "trial", "Other stores"))
 ]
...
percentageDiff <- merge(scaledControlCustomers[, c("YEARMONTH","controlCustomers")],
                        measureOverTimeCusts[STORE_NBR == trial_store,
                                             c("nCustomers", "YEARMONTH")],by = "YEARMONTH", allow.cartesian = TRUE
...
```{r , fig.align = "Center"}
#### As our null hypothesis is that the trial period is the same as the pre-trial period, let's take the standard deviation based on the scaled percentage difference in the pr-trial period
...
measureOverTime <- data[, .(totSales = sum(TOT_SALES),
 nCustomers = uniqueN(LYLTY_CARD_NBR),
 nTxnPerCust = uniqueN(TXN_ID)/unique(LYLTY_CARD_NBR),
 nChipsPerTxn = sum(PROD_QTY)/uniqueN(TXN_ID),
 avgPricePerUnit = sum(TOT_SALES)/sum(PROD_QTY)
 )
 , by = c("STORE_NBR","YEARMONTH") ][order(STORE_NBR, YEARMONTH )]
...
measureOverTimeSales <- measureOverTime
pastSales <- measureOverTimeSales[, Store_type := ifelse(STORE_NBR == control_store, "Control", ifelse(STORE_NBR == trial_store, "Trial", "Other Stores"))
 ][, totSales := mean(totSales) , by = c("YEARMONTH", "Store_type") 
...
#### Conduct visual checks on trends based on the drivers
measureOverTimeCusts <- measureOverTime
pastCustomers <- measureOverTimeCusts[, Store_type := ifelse(STORE_NBR == control_store, "Control", ifelse(STORE_NBR == trial_store, "Trial", "Other Stores"))
 ][, numberCustomers := mean(nCustomers) , by = c("YEARMONTH", "Store_type")
 ][, TransactionMonth := as.Date(paste(YEARMONTH %/%100, YEARMONTH %% 100, 1, sep = "‐"), "%Y‐%m‐%d")
 ][YEARMONTH < 201903 , ]
...
We've now found store 237 to be a suitable control store for trial store 88. Again, let's check visually if the drivers are indeed similar in the period before the trial. Page 15 20200128_InsideSherpa_Task2_DraftSolutions - Template.Rmd We'll look at total sales first.
